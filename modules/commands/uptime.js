...
var time = process.uptime(),
  ...
  var id = Math.floor(Math.random() * 848) +1
    ...
    let background = (await axios.get(encodeURI(`https://i.postimg.cc/d1gYbJ1y/IMG-1705946778432.jpg`), { responseType: "arraybuffer" })).data;
...
registerFont(__dirname + `/cache/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "55px time";
    ctx.fillText("Abdulla Rahaman", 930, 540)
    ctx.fillText("Abdulla Rahaman", 930, 610)
    ctx.fillText("Abdulla Rahaman", 930, 690)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
